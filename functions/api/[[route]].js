...
export async function onRequest(context) {
  ...
  if (route === 'settings') {
    ...
    if (method === 'PUT') {
      ...
      await db.prepare(
        'INSERT INTO settings (user_id, name, role, work_start, work_end, theme, updated_at) VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT(user_id) DO UPDATE SET name = excluded.name, role = excluded.role, work_start = excluded.work_start, work_end = excluded.work_end, theme = excluded.theme, updated_at = CURRENT_TIMESTAMP'
      ).bind(userId, encName, encRole, body.workStart || '9:00 AM', body.workEnd || '6:00 PM', body.theme || 'meridian').run();
      return json({ ok: true });
    }
  }
  ...
  // ── /api/events/past ─────────────────────────────────────────────────────
  if (route === 'events/past') {
    if (method === 'GET') {
      const today = new Date().toISOString().slice(0, 10);
      const { results: pastRows } = await db
        .prepare('SELECT * FROM events WHERE user_id = ? AND event_date < ? ORDER BY event_date DESC, start_time ASC LIMIT 200')
        .bind(userId, today).all();
      const decryptedPast = await Promise.all(pastRows.map(async e => ({
        id:         e.id,
        title:      await decrypt(e.title, cryptoKey),
        start_time: e.start_time,
        end_time:   e.end_time,
